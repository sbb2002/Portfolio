...
while start_year >= 2011:
    ...
    mth_elems = browser.find_elements_by_xpath('//*[@id="regDatepicker"]/div/div/div/select[2]/*')
    for month in range(len(mth_elems)):
        inner_mth_elems = browser.find_elements_by_xpath('//*[@id="regDatepicker"]/div/div/div/select[2]/*')
        mth_count = len(mth_elems) - 1 - month
        mth = p.findall([ e.text for e in inner_mth_elems ][mth_count])[0]
        if int(mth) < 10:
            mth = "0" + mth
        inner_mth_elems[mth_count].click()
